<%-- 
    Document   : table001a
    Created on : 28 Nov, 2018, 12:10:34 PM
    Author     : Sajid
--%>
...
%>
	...
	ResultSet rs = st.executeQuery("select charity_name, sum(qty) from FDA.DELIVERED where calender>='"+a+"' and calender<='"+b+"' group by charity_name order by sum(qty) desc");    
                while(rs.next())
                      {
                          out.println("<tr>");
                          out.println("<td>");
                          out.println(""+rs.getString("charity_name")+"");
                          out.println("</td>");
                          out.println("<td>");
