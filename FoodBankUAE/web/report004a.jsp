<%-- 
    Document   : report004a
    Created on : 28 Nov, 2018, 10:42:45 PM
    Author     : Sajid
--%>
...
%>
  ...
  function makeScreenshot()
	...
	ResultSet rs = statement.executeQuery("SELECT year(CALENDER) as y,sum(QTY) as quantity FROM FDA.RECEIVE where year(CALENDER)>="+a+" and year(CALENDER)<="+b+" group by year(CALENDER)");    
                while(rs.next())
                      {
	...
	ResultSet rs1 = st.executeQuery("SELECT sum(QTY) as q FROM FDA.RECEIVE where year(CALENDER)>="+a+" and year(CALENDER)<="+b+" ");    
                while(rs1.next())
                      {
            ...
            %>,
                    ...
                    labels: [
                        <%
                        ResultSet rss = statement.executeQuery("SELECT year(CALENDER) as y,sum(QTY) as quantity FROM FDA.RECEIVE where year(CALENDER)>="+a+" and year(CALENDER)<="+b+" group by year(CALENDER)");
                    while(rss.next())
                    {
