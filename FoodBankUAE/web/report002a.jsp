<%-- 
    Document   : report002a
    Created on : 28 Nov, 2018, 4:37:52 PM
    Author     : Sajid
--%>
...
%>
  ...
  function makeScreenshot()
	...
	ResultSet rs = statement.executeQuery("select product, sum(qty) from FDA.RECEIVE where SUPPLIER='"+c+"' and  calender>='"+a+"' and calender<='"+b+"' group by product order by sum(qty) desc");    
                while(rs.next())
                      {
	...
	ResultSet rs1 = st.executeQuery("select  sum(qty)  from FDA.RECEIVE where SUPPLIER='"+c+"' and  calender>='"+a+"' and calender<='"+b+"'  ");    
                while(rs1.next())
                      {
            ...
            %>,
                    ...
                    labels: [
                        <%
                        ResultSet rss = statement.executeQuery("select product, sum(qty) from FDA.RECEIVE where SUPPLIER='"+c+"' and  calender>='"+a+"' and calender<='"+b+"' group by product order by sum(qty) desc");
                    while(rss.next())
                    {
                     out.println("'"+rss.getString("product")+"',");
                    }
                    %>
                    ],
                    labelsAbove: true,
