<%-- 
    Document   : report003a
    Created on : 28 Nov, 2018, 5:24:07 PM
    Author     : Sajid
--%>
...
%>
  ...
  function makeScreenshot()
	...
	ResultSet rs = statement.executeQuery("select supplier, sum(qty) from FDA.RECEIVE where calender>='"+a+"' and calender<='"+b+"' group by supplier order by sum(qty) desc");    
                while(rs.next())
                      {
	...
	ResultSet rs1 = st.executeQuery("select  sum(qty) as a from FDA.RECEIVE where calender>='"+a+"' and calender<='"+b+"'");    
                while(rs1.next())
                      {
            ...
            %>,
                    ...
                    labels: [
                        <%
                        ResultSet rss = statement.executeQuery("select supplier, sum(qty) from FDA.RECEIVE where calender>='"+a+"' and calender<='"+b+"' group by supplier order by sum(qty) desc");
                    while(rss.next())
                    {
